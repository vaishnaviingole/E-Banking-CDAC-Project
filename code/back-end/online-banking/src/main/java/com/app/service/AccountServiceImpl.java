...
import com.app.custom_exception.InsufficientBalanceException;
...
//		double amount = Double.parseDouble(amount1);
	...
	@Override
	public void withdraw(String accountType, double amount, String userName) throws InsufficientBalanceException {
	    User user = userService.findByUserName(userName);
	    System.out.println(user);
	    if (accountType.equals("Primary")) {
	        PrimaryAccount primaryAccount = user.getPrimaryAccount();
	        BigDecimal currentBalance = primaryAccount.getAccountBalance();
	        if (currentBalance.compareTo(new BigDecimal(amount)) < 0) {
	            throw new InsufficientBalanceException("Withdrawal amount exceeds account balance");
	        }
	        primaryAccount.setAccountBalance(currentBalance.subtract(new BigDecimal(amount)));
	        primaryAccountRepo.save(primaryAccount);
	        ...
	        savingsAccount.setAccountBalance(currentBalance.subtract(new BigDecimal(amount)));
	        savingsAccountRepo.save(savingsAccount);
